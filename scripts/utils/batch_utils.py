...
class Outlier(object):
    ...
    def __init__(self, data, start_indices, end_indices, fin_colidxs, stride, confidence_level, mean_window,
                 std_window, start_date, end_date, max_unrollings):
        self._data = data
    ...
    def _get_outlier_idxs(self):
        """
        Identifies the outliers based on rolling mean and std dev and returns index of outliers
        :return: indices of outliers
        """
        mean_window = int(self._stride * self.mean_window)
        std_window = int(self._stride * self.std_window)
        ...
        for i, gvkey in enumerate(self._data.gvkey.unique()):
            ...
            # For training, look ahead data is present and hence historical and future data should be used.
            # To calculate the rolling mean and rolling stdev, -window and +window should be used.
            # The series is shifted with -window/2 and then rolling calculation is done.
            roll_mean = output_series.shift(-int(mean_window/2)).rolling(mean_window, min_periods=1).mean()
            dev = z_score * output_series.shift(-int(std_window/2)).rolling(std_window).std()
    ...
    def get_outlier_bounds_for_preds(self):
        """
        ...
        for i, gvkey in enumerate(unique_gvkeys):
            ...
            # Shift the output series by -stride to be consistent with the outputs
            # generated by the prediction. The dates from that process imply when the
            # forecast was made where as the output series directly from the data represents
            # output data on that date.
            output_series = output_series.shift(-self._stride)

            roll_mean = output_series.rolling(mean_window, min_periods=1).mean()
            std = output_series.rolling(std_window).std()
